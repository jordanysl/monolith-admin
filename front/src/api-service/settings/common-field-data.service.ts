...
export default {
  ...
  recycleBin(paginationQuery?: any): Promise<PageRecord<ICommonFieldData[]>> {
    const options = buildPaginationQueryOpts(paginationQuery);
    return defHttp.get({ url: apiUrl + '/recycle-bin' + `?${qs.stringify(options, { arrayFormat: 'repeat' })}` });
  },

  restoreByIds(ids: number[]): Promise<ICommonFieldData> {
    const commonFieldData = { delFlag: false };
    return defHttp.put({ url: `${apiUrl}/specified-field-batch`, params: { commonFieldData, specifiedField: 'delFlag', ids } });
  },

  reallyDelete(ids: number[]): Promise<ICommonFieldData> {
    return defHttp.delete({ url: `${apiUrl}/really-delete`, params: { ids } });
  },

  emptyRecycleBin(): Promise<ICommonFieldData> {
  ...
  batchUpdateByField(commonFieldData: ICommonFieldData, specifiedField: String, ids: number[] | number): Promise<ICommonFieldData> {
    return defHttp.put({ url: `${apiUrl}/field-batch`, params: { commonFieldData, specifiedField, ids } });
  },
