...
export default {
  ...
  recycleBin(paginationQuery?: any): Promise<PageRecord<IPosition[]>> {
    const options = buildPaginationQueryOpts(paginationQuery);
    return defHttp.get({ url: apiUrl + '/recycle-bin' + `?${qs.stringify(options, { arrayFormat: 'repeat' })}` });
  },

  restoreByIds(ids: number[]): Promise<IPosition> {
    const position = { delFlag: false };
    return defHttp.put({ url: `${apiUrl}/specified-field-batch`, params: { position, specifiedField: 'delFlag', ids } });
  },

  reallyDelete(ids: number[]): Promise<IPosition> {
    return defHttp.delete({ url: `${apiUrl}/really-delete`, params: { ids } });
  },

  emptyRecycleBin(): Promise<IPosition> {
  ...
  batchUpdateByField(position: IPosition, specifiedField: String, ids: number[] | number): Promise<IPosition> {
    return defHttp.put({ url: `${apiUrl}/field-batch`, params: { position, specifiedField, ids } });
  },
