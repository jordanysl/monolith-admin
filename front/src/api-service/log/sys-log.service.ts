...
export default {
  ...
  recycleBin(paginationQuery?: any): Promise<PageRecord<ISysLog[]>> {
    const options = buildPaginationQueryOpts(paginationQuery);
    return defHttp.get({ url: apiUrl + '/recycle-bin' + `?${qs.stringify(options, { arrayFormat: 'repeat' })}` });
  },

  restoreByIds(ids: number[]): Promise<ISysLog> {
    const sysLog = { delFlag: false };
    return defHttp.put({ url: `${apiUrl}/specified-field-batch`, params: { sysLog, specifiedField: 'delFlag', ids } });
  },

  reallyDelete(ids: number[]): Promise<ISysLog> {
    return defHttp.delete({ url: `${apiUrl}/really-delete`, params: { ids } });
  },

  emptyRecycleBin(): Promise<ISysLog> {
  ...
  create(sysLog: ISysLog): Promise<ISysLog> {
    return defHttp.post({ url: `${apiUrl}`, params: sysLog });
  },
  update(sysLog: ISysLog, batchIds?: number[], batchFields?: String[]): Promise<ISysLog> {
    ...
    return defHttp.put({ url: `${apiUrl}/${sysLog.id}?${queryParams}`, params: sysLog });
  },

  batchUpdateByField(sysLog: ISysLog, specifiedField: String, ids: number[] | number): Promise<ISysLog> {
    return defHttp.put({ url: `${apiUrl}/field-batch`, params: { sysLog, specifiedField, ids } });
  },
