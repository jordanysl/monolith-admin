...
const apiUrl = '/api/sms-suppliers';

export default {
  // begcode-please-regenerate-this-file 如果您不希望重新生成代码时被覆盖，将please修改为don't ！！！

  find(id: number): Promise<ISmsSupplier> {
    return defHttp.get({ url: `${apiUrl}/${id}` });
  },

  retrieve(paginationQuery?: any): Promise<PageRecord<ISmsSupplier>> {
  ...
  recycleBin(paginationQuery?: any): Promise<PageRecord<ISmsSupplier[]>> {
    const options = buildPaginationQueryOpts(paginationQuery);
    return defHttp.get({ url: apiUrl + '/recycle-bin' + `?${qs.stringify(options, { arrayFormat: 'repeat' })}` });
  },

  restoreByIds(ids: number[]): Promise<ISmsSupplier> {
    const smsSupplier = { delFlag: false };
    return defHttp.put({ url: `${apiUrl}/specified-field-batch`, params: { smsSupplier, specifiedField: 'delFlag', ids } });
  },

  reallyDelete(ids: number[]): Promise<ISmsSupplier> {
    return defHttp.delete({ url: `${apiUrl}/really-delete`, params: { ids } });
  },

  emptyRecycleBin(): Promise<ISmsSupplier> {
  ...
  create(smsSupplier: ISmsSupplier): Promise<ISmsSupplier> {
    return defHttp.post({ url: `${apiUrl}`, params: smsSupplier });
  },
  update(smsSupplier: ISmsSupplier, batchIds?: number[], batchFields?: String[]): Promise<ISmsSupplier> {
    let queryParams = '';
    if (batchIds && batchFields) {
      queryParams = qs.stringify({ batchIds, batchFields }, { arrayFormat: 'repeat' });
    }
    return defHttp.put({ url: `${apiUrl}/${smsSupplier.id}?${queryParams}`, params: smsSupplier });
  },

  batchUpdateByField(smsSupplier: ISmsSupplier, specifiedField: String, ids: number[] | number): Promise<ISmsSupplier> {
    return defHttp.put({ url: `${apiUrl}/field-batch`, params: { smsSupplier, specifiedField, ids } });
  },
