...
export default {
  ...
  recycleBin(paginationQuery?: any): Promise<PageRecord<IUploadFile[]>> {
    const options = buildPaginationQueryOpts(paginationQuery);
    return defHttp.get({ url: apiUrl + '/recycle-bin' + `?${qs.stringify(options, { arrayFormat: 'repeat' })}` });
  },

  restoreByIds(ids: number[]): Promise<IUploadFile> {
    const uploadFile = { delFlag: false };
    return defHttp.put({ url: `${apiUrl}/specified-field-batch`, params: { uploadFile, specifiedField: 'delFlag', ids } });
  },

  reallyDelete(ids: number[]): Promise<IUploadFile> {
    return defHttp.delete({ url: `${apiUrl}/really-delete`, params: { ids } });
  },

  emptyRecycleBin(): Promise<IUploadFile> {
  ...
  update(uploadFile: IUploadFile, batchIds?: number[], batchFields?: String[]): Promise<IUploadFile> {
    ...
    return defHttp.put({ url: `${apiUrl}/${uploadFile.id}?${queryParams}`, params: uploadFile });
  },
  ...
  batchUpdateByField(uploadFile: IUploadFile, specifiedField: String, ids: number[] | number): Promise<IUploadFile> {
    return defHttp.put({ url: `${apiUrl}/field-batch`, params: { uploadFile, specifiedField, ids } });
  },
