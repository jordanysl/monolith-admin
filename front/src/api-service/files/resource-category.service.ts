...
export default {
  ...
  recycleBin(paginationQuery?: any): Promise<PageRecord<IResourceCategory[]>> {
    const options = buildPaginationQueryOpts(paginationQuery);
    return defHttp.get({ url: apiUrl + '/recycle-bin' + `?${qs.stringify(options, { arrayFormat: 'repeat' })}` });
  },

  restoreByIds(ids: number[]): Promise<IResourceCategory> {
    const resourceCategory = { delFlag: false };
    return defHttp.put({ url: `${apiUrl}/specified-field-batch`, params: { resourceCategory, specifiedField: 'delFlag', ids } });
  },

  reallyDelete(ids: number[]): Promise<IResourceCategory> {
    return defHttp.delete({ url: `${apiUrl}/really-delete`, params: { ids } });
  },

  emptyRecycleBin(): Promise<IResourceCategory> {
  ...
  batchUpdateByField(resourceCategory: IResourceCategory, specifiedField: String, ids: number[] | number): Promise<IResourceCategory> {
    return defHttp.put({ url: `${apiUrl}/field-batch`, params: { resourceCategory, specifiedField, ids } });
  },
