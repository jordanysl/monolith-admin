...
export default {
  ...
  recycleBin(paginationQuery?: any): Promise<PageRecord<IOssConfig[]>> {
    const options = buildPaginationQueryOpts(paginationQuery);
    return defHttp.get({ url: apiUrl + '/recycle-bin' + `?${qs.stringify(options, { arrayFormat: 'repeat' })}` });
  },

  restoreByIds(ids: number[]): Promise<IOssConfig> {
    const ossConfig = { delFlag: false };
    return defHttp.put({ url: `${apiUrl}/specified-field-batch`, params: { ossConfig, specifiedField: 'delFlag', ids } });
  },

  reallyDelete(ids: number[]): Promise<IOssConfig> {
    return defHttp.delete({ url: `${apiUrl}/really-delete`, params: { ids } });
  },

  emptyRecycleBin(): Promise<IOssConfig> {
  ...
  batchUpdateByField(ossConfig: IOssConfig, specifiedField: String, ids: number[] | number): Promise<IOssConfig> {
    return defHttp.put({ url: `${apiUrl}/field-batch`, params: { ossConfig, specifiedField, ids } });
  },
