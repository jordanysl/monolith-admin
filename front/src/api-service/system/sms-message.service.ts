...
const apiUrl = '/api/sms-messages';

export default {
  // begcode-please-regenerate-this-file 如果您不希望重新生成代码时被覆盖，将please修改为don't ！！！

  find(id: number): Promise<ISmsMessage> {
    return defHttp.get({ url: `${apiUrl}/${id}` });
  },

  retrieve(paginationQuery?: any): Promise<PageRecord<ISmsMessage>> {
  ...
  recycleBin(paginationQuery?: any): Promise<PageRecord<ISmsMessage[]>> {
    const options = buildPaginationQueryOpts(paginationQuery);
    return defHttp.get({ url: apiUrl + '/recycle-bin' + `?${qs.stringify(options, { arrayFormat: 'repeat' })}` });
  },

  restoreByIds(ids: number[]): Promise<ISmsMessage> {
    const smsMessage = { delFlag: false };
    return defHttp.put({ url: `${apiUrl}/specified-field-batch`, params: { smsMessage, specifiedField: 'delFlag', ids } });
  },

  reallyDelete(ids: number[]): Promise<ISmsMessage> {
    return defHttp.delete({ url: `${apiUrl}/really-delete`, params: { ids } });
  },

  emptyRecycleBin(): Promise<ISmsMessage> {
  ...
  create(smsMessage: ISmsMessage): Promise<ISmsMessage> {
    return defHttp.post({ url: `${apiUrl}`, params: smsMessage });
  },
  update(smsMessage: ISmsMessage, batchIds?: number[], batchFields?: String[]): Promise<ISmsMessage> {
    let queryParams = '';
    if (batchIds && batchFields) {
      queryParams = qs.stringify({ batchIds, batchFields }, { arrayFormat: 'repeat' });
    }
    return defHttp.put({ url: `${apiUrl}/${smsMessage.id}?${queryParams}`, params: smsMessage });
  },

  batchUpdateByField(smsMessage: ISmsMessage, specifiedField: String, ids: number[] | number): Promise<ISmsMessage> {
    return defHttp.put({ url: `${apiUrl}/field-batch`, params: { smsMessage, specifiedField, ids } });
  },
