...
export default defineComponent({
  components: { Icon, BasicModal, Upload, Alert },
  props: {
    ...basicProps,
    previewFileList: {
  ...
  setup(props, { emit }) {
    ...
    function beforeUpload(file: File) {
      ...
      if (maxSize && file.size / 1024 / 1024 >= maxSize) {
        createMessage.error(t('component.upload.maxSizeMultiple', { maxSize: maxSize }));
        return false;
      }

      // 设置类型,则判断
      if (accept.length > 0 && !checkFileType(file, accept)) {
        createMessage.error!(t('component.upload.acceptUpload', { acceptFormat: accept.join(',') }));
        return false;
      }
    ...
    function handleOk() {
      ...
      if (fileListRef.value.length > maxNumber) {
        return createMessage.warning(t('component.upload.maxNumber', { maxNumber: maxNumber }));
      }
