...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (!data) {
      ...
      throw new Error(t('sys.api.apiRequestFailed') || '请求出错，请稍候重试');
    }
    ...
    switch (code) {
      case ResultEnum.TIMEOUT:
        timeoutMsg = t('sys.api.timeoutMessage') || '登录超时,请重新登录!';
    ...
    if (options.errorMessageMode === 'modal') {
      createErrorModal({ title: t('sys.api.errorTip') || '错误提示', content: timeoutMsg });
    } else if (options.errorMessageMode === 'message') {
      createMessage.error(timeoutMsg);
    }

    throw new Error(timeoutMsg || t('sys.api.apiRequestFailed') || '请求出错，请稍候重试');
  },
  ...
  responseInterceptorsCatch: (axiosInstance: AxiosResponse, error: any) => {
    ...
    try {
      ...
      if (code === 'ECONNABORTED' && message.indexOf('timeout') !== -1) {
        errMessage = t('sys.api.apiTimeoutMessage') || '接口请求超时,请刷新页面重试!';
      }
      if (err?.includes('Network Error')) {
        errMessage = t('sys.api.networkExceptionMsg') || '网络异常，请检查您的网络连接是否正常!';
      }
