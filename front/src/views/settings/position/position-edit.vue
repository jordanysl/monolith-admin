...
import { Position, IPosition } from '@/models/settings/position.model';
...
export default defineComponent({
  ...
  async setup(props) {
    ...
    if (props.containerType === 'router') {
      positionId.value = route.params?.entityId as string;
    } else {
      positionId.value = props.entityId as string;
    }
    const position = reactive<IPosition>(new Position());
    if (positionId.value) {
      const data = await apiService.settings.positionService.find(Number(positionId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = positionId.value ? '更新' : '保存';
    const saveOrUpdateApi = positionId.value ? apiService.settings.positionService.update : apiService.settings.positionService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(position, result);
            saveOrUpdateApi(position)
              .then(res => {
                Object.assign(position, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: positionId.value ? '编辑岗位' : '新增岗位',
                  bordered: false,
                  size: 'small',
                }}
