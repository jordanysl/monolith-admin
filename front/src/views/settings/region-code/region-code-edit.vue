...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const relationshipApis: any = {
      ...
      'parent.id': apiService.settings.regionCodeService.tree,
    };
    ...
    let regionCodeId = ref('');
    if (props.containerType === 'router') {
      regionCodeId.value = route.params?.entityId as string;
    } else {
      regionCodeId.value = props.entityId as string;
    }
    const regionCode = reactive<IRegionCode>(new RegionCode());
    if (regionCodeId.value) {
      const data = await apiService.settings.regionCodeService.find(Number(regionCodeId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = regionCodeId.value ? '更新' : '保存';
    const saveOrUpdateApi = regionCodeId.value
      ? apiService.settings.regionCodeService.update
      : apiService.settings.regionCodeService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(regionCode, result);
            saveOrUpdateApi(regionCode)
              .then(res => {
                Object.assign(regionCode, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: regionCodeId.value ? '编辑行政区划码' : '新增行政区划码',
                  bordered: false,
                  size: 'small',
                }}
