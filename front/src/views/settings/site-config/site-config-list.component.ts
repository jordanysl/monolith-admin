...
import SiteConfigEdit from './site-config-edit.vue';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const apis = {
      siteConfigService: apiService.settings.siteConfigService,
      find: apiService.settings.siteConfigService.retrieve,
      deleteById: apiService.settings.siteConfigService.delete,
      deleteByIds: apiService.settings.siteConfigService.deleteByIds,
      update: apiService.settings.siteConfigService.update,
    };
    const pageConfig = {
      title: '网站配置列表',
      baseRouteName: 'systemSiteConfig',
    };
          ...
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'site-config-edit';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'site-config-edit';
        ...
        case 'detail':
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'site-config-detail';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'site-config-detail';
                  this.drawerConfig.entityId = row.id;
                  this.setDrawerProps({ visible: true });
                  break;
                case 'route':
