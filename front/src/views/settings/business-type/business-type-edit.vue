...
export default defineComponent({
  ...
  async setup(props) {
    ...
    let businessTypeId = ref('');
    if (props.containerType === 'router') {
      businessTypeId.value = route.params?.entityId as string;
    } else {
      businessTypeId.value = props.entityId as string;
    }
    const businessType = reactive<IBusinessType>(new BusinessType());
    if (businessTypeId.value) {
      const data = await apiService.settings.businessTypeService.find(Number(businessTypeId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = businessTypeId.value ? '更新' : '保存';
    const saveOrUpdateApi = businessTypeId.value
      ? apiService.settings.businessTypeService.update
      : apiService.settings.businessTypeService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(businessType, result);
            saveOrUpdateApi(businessType)
              .then(res => {
                Object.assign(businessType, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: businessTypeId.value ? '编辑业务类型' : '新增业务类型',
                  bordered: false,
                  size: 'small',
                }}
