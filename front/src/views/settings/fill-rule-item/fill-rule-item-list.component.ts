...
import FillRuleItemEdit from './fill-rule-item-edit.vue';
import FillRuleItemDetail from './fill-rule-item-detail.vue';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const apis = {
      ...
      deleteById: apiService.settings.fillRuleItemService.delete,
      deleteByIds: apiService.settings.fillRuleItemService.deleteByIds,
      update: apiService.settings.fillRuleItemService.update,
    };
    const pageConfig = {
      title: '填充规则条目列表',
      baseRouteName: 'systemFillRuleItem',
    };
          ...
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'fill-rule-item-edit';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'fill-rule-item-edit';
        ...
        case 'detail':
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'fill-rule-item-detail';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'fill-rule-item-detail';
                  this.drawerConfig.entityId = row.id;
                  this.setDrawerProps({ visible: true });
                  break;
                case 'route':
