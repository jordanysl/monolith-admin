...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const relationshipApis: any = {
      'fillRule.id': apiService.settings.sysFillRuleService.retrieve,
    };
    ...
    let fillRuleItemId = ref('');
    if (props.containerType === 'router') {
      fillRuleItemId.value = route.params?.entityId as string;
    } else {
      fillRuleItemId.value = props.entityId as string;
    }
    const fillRuleItem = reactive<IFillRuleItem>(new FillRuleItem());
    if (fillRuleItemId.value) {
      const data = await apiService.settings.fillRuleItemService.find(Number(fillRuleItemId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = fillRuleItemId.value ? '更新' : '保存';
    const saveOrUpdateApi = fillRuleItemId.value
      ? apiService.settings.fillRuleItemService.update
      : apiService.settings.fillRuleItemService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(fillRuleItem, result);
            saveOrUpdateApi(fillRuleItem)
              .then(res => {
                Object.assign(fillRuleItem, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: fillRuleItemId.value ? '编辑填充规则条目' : '新增填充规则条目',
                  bordered: false,
                  size: 'small',
                }}
