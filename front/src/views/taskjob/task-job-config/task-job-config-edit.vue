...
export default defineComponent({
  ...
  async setup(props) {
    ...
    let taskJobConfigId = ref('');
    if (props.containerType === 'router') {
      taskJobConfigId.value = route.params?.entityId as string;
    } else {
      taskJobConfigId.value = props.entityId as string;
    }
    const taskJobConfig = reactive<ITaskJobConfig>(new TaskJobConfig());
    if (taskJobConfigId.value) {
      const data = await apiService.taskjob.taskJobConfigService.find(Number(taskJobConfigId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = taskJobConfigId.value ? '更新' : '保存';
    const saveOrUpdateApi = taskJobConfigId.value
      ? apiService.taskjob.taskJobConfigService.update
      : apiService.taskjob.taskJobConfigService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(taskJobConfig, result);
            saveOrUpdateApi(taskJobConfig)
              .then(res => {
                Object.assign(taskJobConfig, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: taskJobConfigId.value ? '编辑定时任务' : '新增定时任务',
                  bordered: false,
                  size: 'small',
                }}
