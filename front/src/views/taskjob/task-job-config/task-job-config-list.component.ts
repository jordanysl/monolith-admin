...
import TaskJobConfigEdit from './task-job-config-edit.vue';
import TaskJobConfigDetail from './task-job-config-detail.vue';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const apis = {
      taskJobConfigService: apiService.taskjob.taskJobConfigService,
      find: apiService.taskjob.taskJobConfigService.retrieve,
      deleteById: apiService.taskjob.taskJobConfigService.delete,
      deleteByIds: apiService.taskjob.taskJobConfigService.deleteByIds,
      update: apiService.taskjob.taskJobConfigService.update,
    };
    const pageConfig = {
      title: '定时任务列表',
      baseRouteName: 'taskjobTaskJobConfig',
    };
          ...
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'task-job-config-edit';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'task-job-config-edit';
        ...
        case 'detail':
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'task-job-config-detail';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'task-job-config-detail';
                  this.drawerConfig.entityId = row.id;
                  this.setDrawerProps({ visible: true });
                  break;
                case 'route':
