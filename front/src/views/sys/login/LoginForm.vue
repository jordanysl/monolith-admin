<template>
  ...
  <Form class="p-4 enter-x" :model="formData" :rules="getFormRules" ref="formRef" v-show="getShow" @keypress.enter="handleLogin">
    <FormItem name="account" class="enter-x">
      <Input size="large" v-model:value="formData.account" :placeholder="t('sys.login.userName') || '账号'" class="fix-auto-fill" />
    </FormItem>
    <FormItem name="password" class="enter-x">
      <InputPassword size="large" visibilityToggle v-model:value="formData.password" :placeholder="t('sys.login.password') || '密码'" />
    </FormItem>
    <!--验证码-->
    <ARow class="enter-x">
      <ACol :span="12">
        <FormItem name="inputCode" class="enter-x">
          <Input
            ...
            :placeholder="t('sys.login.inputCode') || '验证码'"
      ...
      <ACol :span="12">
        <FormItem :style="{ 'text-align': 'right' }">
          ...
          <Button type="link" size="small" @click="setLoginState(LoginStateEnum.RESET_PASSWORD)">
            {{ t('sys.login.forgetPassword') || '忘记密码?' }}
          </Button>
        </FormItem>
      </ACol>
    </ARow>
    ...
    <ARow class="enter-x">
      <ACol :md="8" :xs="24">
        <Button block @click="setLoginState(LoginStateEnum.MOBILE)">
          {{ t('sys.login.mobileSignInFormTitle') || '手机登录' }}
        </Button>
      </ACol>
      <ACol :md="8" :xs="24" class="!my-2 !md:my-0 xs:mx-0 md:mx-2">
        <Button block @click="setLoginState(LoginStateEnum.QR_CODE)">
          {{ t('sys.login.qrSignInFormTitle') || '二维码登录' }}
        </Button>
      </ACol>
      <ACol :md="6" :xs="24">
    ...
    <Divider class="enter-x">{{ t('sys.login.otherSignIn') || '其他登录方式' }}</Divider>
  ...
  try {
    ...
    if (userInfo) {
      notification.success({
        message: t('sys.login.loginSuccessTitle') || '登录成功',
        description: `${t('sys.login.loginSuccessDesc') || '欢迎回来'}: ${userInfo.firstName}`,
        duration: 3,
      });
    }
  } catch (error) {
