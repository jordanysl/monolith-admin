...
export default defineComponent({
  ...
  async setup(props) {
    ...
    if (props.containerType === 'router') {
      sysLogId.value = route.params?.entityId as string;
    } else {
      sysLogId.value = props.entityId as string;
    }
    const sysLog = reactive<ISysLog>(new SysLog());
    if (sysLogId.value) {
      const data = await apiService.log.sysLogService.find(Number(sysLogId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = sysLogId.value ? '更新' : '保存';
    const saveOrUpdateApi = sysLogId.value ? apiService.log.sysLogService.update : apiService.log.sysLogService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(sysLog, result);
            saveOrUpdateApi(sysLog)
              .then(res => {
                Object.assign(sysLog, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: sysLogId.value ? '编辑系统日志' : '新增系统日志',
                  bordered: false,
                  size: 'small',
                }}
