...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const uReportFile = reactive<IUReportFile>(new UReportFile());
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(uReportFile, result);
            saveOrUpdateApi(uReportFile)
              .then(res => {
                Object.assign(uReportFile, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: uReportFileId.value ? '编辑报表存储' : '新增报表存储',
                  bordered: false,
                  size: 'small',
                }}
