...
import { SmsTemplate, ISmsTemplate } from '@/models/system/sms-template.model';
...
export default defineComponent({
  ...
  async setup(props) {
    ...
    let smsTemplateId = ref('');
    if (props.containerType === 'router') {
      smsTemplateId.value = route.params?.entityId as string;
    } else {
      smsTemplateId.value = props.entityId as string;
    }
    const smsTemplate = reactive<ISmsTemplate>(new SmsTemplate());
    if (smsTemplateId.value) {
      const data = await apiService.system.smsTemplateService.find(Number(smsTemplateId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = smsTemplateId.value ? '更新' : '保存';
    const saveOrUpdateApi = smsTemplateId.value ? apiService.system.smsTemplateService.update : apiService.system.smsTemplateService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(smsTemplate, result);
            saveOrUpdateApi(smsTemplate)
              .then(res => {
                Object.assign(smsTemplate, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: smsTemplateId.value ? '编辑消息模板' : '新增消息模板',
                  bordered: false,
                  size: 'small',
                }}
