...
import SmsTemplateDetail from './sms-template-detail.vue';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const apis = {
      smsTemplateService: apiService.system.smsTemplateService,
      find: apiService.system.smsTemplateService.retrieve,
      deleteById: apiService.system.smsTemplateService.delete,
      deleteByIds: apiService.system.smsTemplateService.deleteByIds,
      update: apiService.system.smsTemplateService.update,
    };
    const pageConfig = {
      title: '消息模板列表',
      baseRouteName: 'systemSmsTemplate',
    };
          ...
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'sms-template-edit';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'sms-template-edit';
        ...
        case 'detail':
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'sms-template-detail';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'sms-template-detail';
                  this.drawerConfig.entityId = row.id;
                  this.setDrawerProps({ visible: true });
                  break;
                case 'route':
