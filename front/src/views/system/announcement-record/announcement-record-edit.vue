  ...
  async setup(props) {
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(announcementRecord, result);
            saveOrUpdateApi(announcementRecord)
              .then(res => {
                Object.assign(announcementRecord, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: announcementRecordId.value ? '编辑通告阅读记录' : '新增通告阅读记录',
                  bordered: false,
                  size: 'small',
                }}
