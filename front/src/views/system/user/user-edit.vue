...
import { User, IUser } from '@/models/system/user.model';
...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const relationshipApis: any = {
      'department.id': apiService.settings.departmentService.tree,
      'position.id': apiService.settings.positionService.retrieve,
    ...
    if (props.containerType === 'router') {
      userId.value = route.params?.entityId as string;
    } else {
      userId.value = props.entityId as string;
    }
    const user = reactive<IUser>(new User());
    if (userId.value) {
      const data = await apiService.system.userService.find(Number(userId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = userId.value ? '更新' : '保存';
    const saveOrUpdateApi = userId.value ? apiService.system.userService.update : apiService.system.userService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(user, result);
            saveOrUpdateApi(user)
              .then(res => {
