...
import ApiPermissionEdit from './api-permission-edit.vue';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const apis = {
      ...
      deleteById: apiService.system.apiPermissionService.delete,
      deleteByIds: apiService.system.apiPermissionService.deleteByIds,
      update: apiService.system.apiPermissionService.update,
    };
    const pageConfig = {
      title: 'API权限列表',
      baseRouteName: 'systemApiPermission',
    };
    ...
    const gridOptions = reactive<VxeGridProps>({
      ...
      toolbarConfig: {
        ...
        tools: [
          ...
          { code: 'generate', name: '', circle: false, icon: 'vxe-icon-refresh' },
        ],
      },
      columns,
    });
      ...
      toolbarToolClick({ code }) {
        switch (code) {
          case 'new':
            ...
            break;
          case 'generate': {
            this.apis.apiPermissionService.generate().then(res => {
          ...
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'api-permission-edit';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'api-permission-edit';
        ...
        case 'detail':
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'api-permission-detail';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'api-permission-detail';
                  this.drawerConfig.entityId = row.id;
                  this.setDrawerProps({ visible: true });
                  break;
                case 'route':
