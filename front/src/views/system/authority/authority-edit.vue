...
import { Authority, IAuthority } from '@/models/system/authority.model';
...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const relationshipApis: any = {
      ...
      'parent.id': apiService.system.authorityService.tree,
    ...
    if (props.containerType === 'router') {
      authorityId.value = route.params?.entityId as string;
    } else {
      authorityId.value = props.entityId as string;
    }
    const authority = reactive<IAuthority>(new Authority());
    if (authorityId.value) {
      const data = await apiService.system.authorityService.find(Number(authorityId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = authorityId.value ? '更新' : '保存';
    const saveOrUpdateApi = authorityId.value ? apiService.system.authorityService.update : apiService.system.authorityService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(authority, result);
            saveOrUpdateApi(authority)
              .then(res => {
                Object.assign(authority, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: authorityId.value ? '编辑角色' : '新增角色',
                  bordered: false,
                  size: 'small',
                }}
