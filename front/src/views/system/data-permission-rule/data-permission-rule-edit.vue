...
import { DataPermissionRule, IDataPermissionRule } from '@/models/system/data-permission-rule.model';
...
export default defineComponent({
  ...
  async setup(props) {
    ...
    let dataPermissionRuleId = ref('');
    if (props.containerType === 'router') {
      dataPermissionRuleId.value = route.params?.entityId as string;
    } else {
      dataPermissionRuleId.value = props.entityId as string;
    }
    const dataPermissionRule = reactive<IDataPermissionRule>(new DataPermissionRule());
    if (dataPermissionRuleId.value) {
      const data = await apiService.system.dataPermissionRuleService.find(Number(dataPermissionRuleId.value));
      if (data) {
        Object.assign(dataPermissionRule, data);
      }
    }
    ...
    const submitButtonTitlePrefix = dataPermissionRuleId.value ? '更新' : '保存';
    const saveOrUpdateApi = dataPermissionRuleId.value
      ? apiService.system.dataPermissionRuleService.update
      : apiService.system.dataPermissionRuleService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(dataPermissionRule, result);
            saveOrUpdateApi(dataPermissionRule)
              .then(res => {
                Object.assign(dataPermissionRule, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: dataPermissionRuleId.value ? '编辑数据权限规则' : '新增数据权限规则',
                  bordered: false,
                  size: 'small',
                }}
