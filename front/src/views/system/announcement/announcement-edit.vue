...
import { Announcement, IAnnouncement } from '@/models/system/announcement.model';
...
export default defineComponent({
  ...
  async setup(props) {
    ...
    let announcementId = ref('');
    if (props.containerType === 'router') {
      announcementId.value = route.params?.entityId as string;
    } else {
      announcementId.value = props.entityId as string;
    }
    const announcement = reactive<IAnnouncement>(new Announcement());
    if (announcementId.value) {
      const data = await apiService.system.announcementService.find(Number(announcementId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = announcementId.value ? '更新' : '保存';
    const saveOrUpdateApi = announcementId.value
      ? apiService.system.announcementService.update
      : apiService.system.announcementService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(announcement, result);
            saveOrUpdateApi(announcement)
              .then(res => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: announcementId.value ? '编辑系统通告' : '新增系统通告',
                  bordered: false,
                  size: 'small',
                }}
