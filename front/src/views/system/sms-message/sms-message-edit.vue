...
export default defineComponent({
  ...
  async setup(props) {
    ...
    let smsMessageId = ref('');
    if (props.containerType === 'router') {
      smsMessageId.value = route.params?.entityId as string;
    } else {
      smsMessageId.value = props.entityId as string;
    }
    const smsMessage = reactive<ISmsMessage>(new SmsMessage());
    if (smsMessageId.value) {
      const data = await apiService.system.smsMessageService.find(Number(smsMessageId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = smsMessageId.value ? '更新' : '保存';
    const saveOrUpdateApi = smsMessageId.value ? apiService.system.smsMessageService.update : apiService.system.smsMessageService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(smsMessage, result);
            saveOrUpdateApi(smsMessage)
              .then(res => {
                Object.assign(smsMessage, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: smsMessageId.value ? '编辑短信消息' : '新增短信消息',
                  bordered: false,
                  size: 'small',
                }}
