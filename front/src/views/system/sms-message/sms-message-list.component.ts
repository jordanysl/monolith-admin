...
import SmsMessageEdit from './sms-message-edit.vue';
import SmsMessageDetail from './sms-message-detail.vue';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const apis = {
      smsMessageService: apiService.system.smsMessageService,
      find: apiService.system.smsMessageService.retrieve,
      deleteById: apiService.system.smsMessageService.delete,
      deleteByIds: apiService.system.smsMessageService.deleteByIds,
      update: apiService.system.smsMessageService.update,
    };
    const pageConfig = {
      title: '短信消息列表',
      baseRouteName: 'systemSmsMessage',
    };
          ...
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'sms-message-edit';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'sms-message-edit';
        ...
        case 'detail':
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'sms-message-detail';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'sms-message-detail';
                  this.drawerConfig.entityId = row.id;
                  this.setDrawerProps({ visible: true });
                  break;
                case 'route':
