...
import OssConfigEdit from './oss-config-edit.vue';
import OssConfigDetail from './oss-config-detail.vue';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const apis = {
      ossConfigService: apiService.files.ossConfigService,
      find: apiService.files.ossConfigService.retrieve,
      deleteById: apiService.files.ossConfigService.delete,
      deleteByIds: apiService.files.ossConfigService.deleteByIds,
      update: apiService.files.ossConfigService.update,
    };
    const pageConfig = {
      title: '对象存储配置列表',
      baseRouteName: 'ossOssConfig',
    };
          ...
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'oss-config-edit';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'oss-config-edit';
        ...
        case 'detail':
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'oss-config-detail';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'oss-config-detail';
                  this.drawerConfig.entityId = row.id;
                  this.setDrawerProps({ visible: true });
                  break;
                case 'route':
