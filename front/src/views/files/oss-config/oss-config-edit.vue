...
import { OssConfig, IOssConfig } from '@/models/files/oss-config.model';
...
export default defineComponent({
  ...
  async setup(props) {
    ...
    if (props.containerType === 'router') {
      ossConfigId.value = route.params?.entityId as string;
    } else {
      ossConfigId.value = props.entityId as string;
    }
    const ossConfig = reactive<IOssConfig>(new OssConfig());
    if (ossConfigId.value) {
      const data = await apiService.files.ossConfigService.find(Number(ossConfigId.value));
      if (data) {
    ...
    const submitButtonTitlePrefix = ossConfigId.value ? '更新' : '保存';
    const saveOrUpdateApi = ossConfigId.value ? apiService.files.ossConfigService.update : apiService.files.ossConfigService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(ossConfig, result);
            saveOrUpdateApi(ossConfig)
              .then(res => {
                Object.assign(ossConfig, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: ossConfigId.value ? '编辑对象存储配置' : '新增对象存储配置',
                  bordered: false,
                  size: 'small',
                }}
