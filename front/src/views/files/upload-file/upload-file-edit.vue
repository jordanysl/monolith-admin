...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const relationshipApis: any = {
      'category.id': apiService.files.resourceCategoryService.tree,
    };
    ...
    const uploadFile = reactive<IUploadFile>(new UploadFile());
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(uploadFile, result);
            saveOrUpdateApi(uploadFile)
              .then(res => {
                Object.assign(uploadFile, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: uploadFileId.value ? '编辑上传文件' : '新增上传文件',
                  bordered: false,
                  size: 'small',
                }}
