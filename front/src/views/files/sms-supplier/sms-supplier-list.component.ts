...
import SmsSupplierEdit from './sms-supplier-edit.vue';
import SmsSupplierDetail from './sms-supplier-detail.vue';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const apis = {
      smsSupplierService: apiService.files.smsSupplierService,
      find: apiService.files.smsSupplierService.retrieve,
      deleteById: apiService.files.smsSupplierService.delete,
      deleteByIds: apiService.files.smsSupplierService.deleteByIds,
      update: apiService.files.smsSupplierService.update,
    };
    const pageConfig = {
      title: '短信服务配置列表',
      baseRouteName: 'systemSmsSupplier',
    };
          ...
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'sms-supplier-edit';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'sms-supplier-edit';
        ...
        case 'detail':
          if (operation) {
            if (operation.click) {
              ...
              switch (operation.containerType) {
                case 'modal':
                  this.modalConfig.componentName = 'sms-supplier-detail';
                  this.modalConfig.entityId = row.id;
                  this.setModalProps({ visible: true });
                  break;
                case 'drawer':
                  this.drawerConfig.componentName = 'sms-supplier-detail';
                  this.drawerConfig.entityId = row.id;
                  this.setDrawerProps({ visible: true });
                  break;
                case 'route':
