  ...
  async setup(props) {
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(smsSupplier, result);
            saveOrUpdateApi(smsSupplier)
              .then(res => {
                Object.assign(smsSupplier, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: smsSupplierId.value ? '编辑短信服务配置' : '新增短信服务配置',
                  bordered: false,
                  size: 'small',
                }}
