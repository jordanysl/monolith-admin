...
import { SmsConfig, ISmsConfig } from '@/models/files/sms-config.model';
...
export default defineComponent({
  ...
  async setup(props) {
    ...
    if (props.containerType === 'router') {
      smsConfigId.value = route.params?.entityId as string;
    } else {
      smsConfigId.value = props.entityId as string;
    }
    const smsConfig = reactive<ISmsConfig>(new SmsConfig());
    if (smsConfigId.value) {
      const data = await apiService.files.smsConfigService.find(Number(smsConfigId.value));
      if (data) {
        Object.assign(smsConfig, data);
      }
    }
    ...
    const submitButtonTitlePrefix = smsConfigId.value ? '更新' : '保存';
    const saveOrUpdateApi = smsConfigId.value ? apiService.files.smsConfigService.update : apiService.files.smsConfigService.create;
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            Object.assign(smsConfig, result);
            saveOrUpdateApi(smsConfig)
              .then(res => {
                Object.assign(smsConfig, res);
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: smsConfigId.value ? '编辑短信配置' : '新增短信配置',
                  bordered: false,
                  size: 'small',
                }}
