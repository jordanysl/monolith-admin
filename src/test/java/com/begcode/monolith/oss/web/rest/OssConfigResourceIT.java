...
public class OssConfigResourceIT {
    ...
    void createOssConfig() throws Exception {
        ...
        assertThat(testOssConfig.getDelFlag()).isEqualTo(DEFAULT_DEL_FLAG);
        assertThat(testOssConfig.getDeletedTime()).isEqualTo(DEFAULT_DELETED_TIME);
    }
    ...
    @Test
    @Transactional
    void getAllOssConfigsByDelFlagIsEqualToSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where delFlag equals to DEFAULT_DEL_FLAG
        defaultOssConfigShouldBeFound("delFlag.equals=" + DEFAULT_DEL_FLAG);

        // Get all the ossConfigList where delFlag equals to UPDATED_DEL_FLAG
        defaultOssConfigShouldNotBeFound("delFlag.equals=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllOssConfigsByDelFlagIsInShouldWork() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where delFlag in DEFAULT_DEL_FLAG or UPDATED_DEL_FLAG
        defaultOssConfigShouldBeFound("delFlag.in=" + DEFAULT_DEL_FLAG + "," + UPDATED_DEL_FLAG);

        // Get all the ossConfigList where delFlag equals to UPDATED_DEL_FLAG
        defaultOssConfigShouldNotBeFound("delFlag.in=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllOssConfigsByDelFlagIsNullOrNotNull() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where delFlag is not null
        defaultOssConfigShouldBeFound("delFlag.specified=true");

        // Get all the ossConfigList where delFlag is null
        defaultOssConfigShouldNotBeFound("delFlag.specified=false");
    }

    @Test
    @Transactional
    void getAllOssConfigsByDeletedTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where deletedTime equals to DEFAULT_DELETED_TIME
        defaultOssConfigShouldBeFound("deletedTime.equals=" + DEFAULT_DELETED_TIME);

        // Get all the ossConfigList where deletedTime equals to UPDATED_DELETED_TIME
        defaultOssConfigShouldNotBeFound("deletedTime.equals=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllOssConfigsByDeletedTimeIsInShouldWork() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where deletedTime in DEFAULT_DELETED_TIME or UPDATED_DELETED_TIME
        defaultOssConfigShouldBeFound("deletedTime.in=" + DEFAULT_DELETED_TIME + "," + UPDATED_DELETED_TIME);

        // Get all the ossConfigList where deletedTime equals to UPDATED_DELETED_TIME
        defaultOssConfigShouldNotBeFound("deletedTime.in=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllOssConfigsByDeletedTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where deletedTime is not null
        defaultOssConfigShouldBeFound("deletedTime.specified=true");

        // Get all the ossConfigList where deletedTime is null
        defaultOssConfigShouldNotBeFound("deletedTime.specified=false");
    }

    @Test
    @Transactional
    void getAllOssConfigsByDeletedTimeIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where deletedTime is greater than or equal to DEFAULT_DELETED_TIME
        defaultOssConfigShouldBeFound("deletedTime.greaterThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the ossConfigList where deletedTime is greater than or equal to UPDATED_DELETED_TIME
        defaultOssConfigShouldNotBeFound("deletedTime.greaterThanOrEqual=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllOssConfigsByDeletedTimeIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where deletedTime is less than or equal to DEFAULT_DELETED_TIME
        defaultOssConfigShouldBeFound("deletedTime.lessThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the ossConfigList where deletedTime is less than or equal to SMALLER_DELETED_TIME
        defaultOssConfigShouldNotBeFound("deletedTime.lessThanOrEqual=" + SMALLER_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllOssConfigsByDeletedTimeIsLessThanSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where deletedTime is less than DEFAULT_DELETED_TIME
        defaultOssConfigShouldNotBeFound("deletedTime.lessThan=" + DEFAULT_DELETED_TIME);

        // Get all the ossConfigList where deletedTime is less than UPDATED_DELETED_TIME
        defaultOssConfigShouldBeFound("deletedTime.lessThan=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllOssConfigsByDeletedTimeIsGreaterThanSomething() throws Exception {
        // Initialize the database
        ossConfigRepository.save(ossConfig);

        // Get all the ossConfigList where deletedTime is greater than DEFAULT_DELETED_TIME
        defaultOssConfigShouldNotBeFound("deletedTime.greaterThan=" + DEFAULT_DELETED_TIME);

        // Get all the ossConfigList where deletedTime is greater than SMALLER_DELETED_TIME
        defaultOssConfigShouldBeFound("deletedTime.greaterThan=" + SMALLER_DELETED_TIME);
    }
    ...
    void putExistingOssConfig() throws Exception {
        ...
        assertThat(testOssConfig.getDelFlag()).isEqualTo(UPDATED_DEL_FLAG);
        assertThat(testOssConfig.getDeletedTime()).isEqualTo(UPDATED_DELETED_TIME);
    }
