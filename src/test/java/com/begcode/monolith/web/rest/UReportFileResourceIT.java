...
public class UReportFileResourceIT {
    ...
    void createUReportFile() throws Exception {
        ...
        assertThat(testUReportFile.getDelFlag()).isEqualTo(DEFAULT_DEL_FLAG);
        assertThat(testUReportFile.getDeletedTime()).isEqualTo(DEFAULT_DELETED_TIME);
    }
    ...
    @Test
    @Transactional
    void getAllUReportFilesByDelFlagIsEqualToSomething() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where delFlag equals to DEFAULT_DEL_FLAG
        defaultUReportFileShouldBeFound("delFlag.equals=" + DEFAULT_DEL_FLAG);

        // Get all the uReportFileList where delFlag equals to UPDATED_DEL_FLAG
        defaultUReportFileShouldNotBeFound("delFlag.equals=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllUReportFilesByDelFlagIsInShouldWork() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where delFlag in DEFAULT_DEL_FLAG or UPDATED_DEL_FLAG
        defaultUReportFileShouldBeFound("delFlag.in=" + DEFAULT_DEL_FLAG + "," + UPDATED_DEL_FLAG);

        // Get all the uReportFileList where delFlag equals to UPDATED_DEL_FLAG
        defaultUReportFileShouldNotBeFound("delFlag.in=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllUReportFilesByDelFlagIsNullOrNotNull() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where delFlag is not null
        defaultUReportFileShouldBeFound("delFlag.specified=true");

        // Get all the uReportFileList where delFlag is null
        defaultUReportFileShouldNotBeFound("delFlag.specified=false");
    }

    @Test
    @Transactional
    void getAllUReportFilesByDeletedTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where deletedTime equals to DEFAULT_DELETED_TIME
        defaultUReportFileShouldBeFound("deletedTime.equals=" + DEFAULT_DELETED_TIME);

        // Get all the uReportFileList where deletedTime equals to UPDATED_DELETED_TIME
        defaultUReportFileShouldNotBeFound("deletedTime.equals=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllUReportFilesByDeletedTimeIsInShouldWork() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where deletedTime in DEFAULT_DELETED_TIME or UPDATED_DELETED_TIME
        defaultUReportFileShouldBeFound("deletedTime.in=" + DEFAULT_DELETED_TIME + "," + UPDATED_DELETED_TIME);

        // Get all the uReportFileList where deletedTime equals to UPDATED_DELETED_TIME
        defaultUReportFileShouldNotBeFound("deletedTime.in=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllUReportFilesByDeletedTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where deletedTime is not null
        defaultUReportFileShouldBeFound("deletedTime.specified=true");

        // Get all the uReportFileList where deletedTime is null
        defaultUReportFileShouldNotBeFound("deletedTime.specified=false");
    }

    @Test
    @Transactional
    void getAllUReportFilesByDeletedTimeIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where deletedTime is greater than or equal to DEFAULT_DELETED_TIME
        defaultUReportFileShouldBeFound("deletedTime.greaterThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the uReportFileList where deletedTime is greater than or equal to UPDATED_DELETED_TIME
        defaultUReportFileShouldNotBeFound("deletedTime.greaterThanOrEqual=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllUReportFilesByDeletedTimeIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where deletedTime is less than or equal to DEFAULT_DELETED_TIME
        defaultUReportFileShouldBeFound("deletedTime.lessThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the uReportFileList where deletedTime is less than or equal to SMALLER_DELETED_TIME
        defaultUReportFileShouldNotBeFound("deletedTime.lessThanOrEqual=" + SMALLER_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllUReportFilesByDeletedTimeIsLessThanSomething() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where deletedTime is less than DEFAULT_DELETED_TIME
        defaultUReportFileShouldNotBeFound("deletedTime.lessThan=" + DEFAULT_DELETED_TIME);

        // Get all the uReportFileList where deletedTime is less than UPDATED_DELETED_TIME
        defaultUReportFileShouldBeFound("deletedTime.lessThan=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllUReportFilesByDeletedTimeIsGreaterThanSomething() throws Exception {
        // Initialize the database
        uReportFileRepository.save(uReportFile);

        // Get all the uReportFileList where deletedTime is greater than DEFAULT_DELETED_TIME
        defaultUReportFileShouldNotBeFound("deletedTime.greaterThan=" + DEFAULT_DELETED_TIME);

        // Get all the uReportFileList where deletedTime is greater than SMALLER_DELETED_TIME
        defaultUReportFileShouldBeFound("deletedTime.greaterThan=" + SMALLER_DELETED_TIME);
    }
    ...
    void putExistingUReportFile() throws Exception {
        ...
        assertThat(testUReportFile.getDelFlag()).isEqualTo(UPDATED_DEL_FLAG);
        assertThat(testUReportFile.getDeletedTime()).isEqualTo(UPDATED_DELETED_TIME);
    }
    ...
    void partialUpdateUReportFileWithPatch() throws Exception {
        ...
        partialUpdatedUReportFile.name(UPDATED_NAME).createAt(UPDATED_CREATE_AT).updateAt(UPDATED_UPDATE_AT).delFlag(UPDATED_DEL_FLAG);
