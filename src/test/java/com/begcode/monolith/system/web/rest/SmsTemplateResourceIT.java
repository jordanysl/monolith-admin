...
public class SmsTemplateResourceIT {
    ...
    void createSmsTemplate() throws Exception {
        ...
        assertThat(testSmsTemplate.getDelFlag()).isEqualTo(DEFAULT_DEL_FLAG);
        assertThat(testSmsTemplate.getDeletedTime()).isEqualTo(DEFAULT_DELETED_TIME);
    }
    ...
    @Test
    @Transactional
    void getAllSmsTemplatesByDelFlagIsEqualToSomething() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where delFlag equals to DEFAULT_DEL_FLAG
        defaultSmsTemplateShouldBeFound("delFlag.equals=" + DEFAULT_DEL_FLAG);

        // Get all the smsTemplateList where delFlag equals to UPDATED_DEL_FLAG
        defaultSmsTemplateShouldNotBeFound("delFlag.equals=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllSmsTemplatesByDelFlagIsInShouldWork() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where delFlag in DEFAULT_DEL_FLAG or UPDATED_DEL_FLAG
        defaultSmsTemplateShouldBeFound("delFlag.in=" + DEFAULT_DEL_FLAG + "," + UPDATED_DEL_FLAG);

        // Get all the smsTemplateList where delFlag equals to UPDATED_DEL_FLAG
        defaultSmsTemplateShouldNotBeFound("delFlag.in=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllSmsTemplatesByDelFlagIsNullOrNotNull() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where delFlag is not null
        defaultSmsTemplateShouldBeFound("delFlag.specified=true");

        // Get all the smsTemplateList where delFlag is null
        defaultSmsTemplateShouldNotBeFound("delFlag.specified=false");
    }

    @Test
    @Transactional
    void getAllSmsTemplatesByDeletedTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where deletedTime equals to DEFAULT_DELETED_TIME
        defaultSmsTemplateShouldBeFound("deletedTime.equals=" + DEFAULT_DELETED_TIME);

        // Get all the smsTemplateList where deletedTime equals to UPDATED_DELETED_TIME
        defaultSmsTemplateShouldNotBeFound("deletedTime.equals=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSmsTemplatesByDeletedTimeIsInShouldWork() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where deletedTime in DEFAULT_DELETED_TIME or UPDATED_DELETED_TIME
        defaultSmsTemplateShouldBeFound("deletedTime.in=" + DEFAULT_DELETED_TIME + "," + UPDATED_DELETED_TIME);

        // Get all the smsTemplateList where deletedTime equals to UPDATED_DELETED_TIME
        defaultSmsTemplateShouldNotBeFound("deletedTime.in=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSmsTemplatesByDeletedTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where deletedTime is not null
        defaultSmsTemplateShouldBeFound("deletedTime.specified=true");

        // Get all the smsTemplateList where deletedTime is null
        defaultSmsTemplateShouldNotBeFound("deletedTime.specified=false");
    }

    @Test
    @Transactional
    void getAllSmsTemplatesByDeletedTimeIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where deletedTime is greater than or equal to DEFAULT_DELETED_TIME
        defaultSmsTemplateShouldBeFound("deletedTime.greaterThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the smsTemplateList where deletedTime is greater than or equal to UPDATED_DELETED_TIME
        defaultSmsTemplateShouldNotBeFound("deletedTime.greaterThanOrEqual=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSmsTemplatesByDeletedTimeIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where deletedTime is less than or equal to DEFAULT_DELETED_TIME
        defaultSmsTemplateShouldBeFound("deletedTime.lessThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the smsTemplateList where deletedTime is less than or equal to SMALLER_DELETED_TIME
        defaultSmsTemplateShouldNotBeFound("deletedTime.lessThanOrEqual=" + SMALLER_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSmsTemplatesByDeletedTimeIsLessThanSomething() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where deletedTime is less than DEFAULT_DELETED_TIME
        defaultSmsTemplateShouldNotBeFound("deletedTime.lessThan=" + DEFAULT_DELETED_TIME);

        // Get all the smsTemplateList where deletedTime is less than UPDATED_DELETED_TIME
        defaultSmsTemplateShouldBeFound("deletedTime.lessThan=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSmsTemplatesByDeletedTimeIsGreaterThanSomething() throws Exception {
        // Initialize the database
        smsTemplateRepository.save(smsTemplate);

        // Get all the smsTemplateList where deletedTime is greater than DEFAULT_DELETED_TIME
        defaultSmsTemplateShouldNotBeFound("deletedTime.greaterThan=" + DEFAULT_DELETED_TIME);

        // Get all the smsTemplateList where deletedTime is greater than SMALLER_DELETED_TIME
        defaultSmsTemplateShouldBeFound("deletedTime.greaterThan=" + SMALLER_DELETED_TIME);
    }
    ...
    void putExistingSmsTemplate() throws Exception {
        ...
        assertThat(testSmsTemplate.getDelFlag()).isEqualTo(UPDATED_DEL_FLAG);
        assertThat(testSmsTemplate.getDeletedTime()).isEqualTo(UPDATED_DELETED_TIME);
    }
