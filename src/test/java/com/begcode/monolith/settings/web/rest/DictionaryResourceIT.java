...
import com.begcode.monolith.settings.web.rest.CommonFieldDataResourceIT;
...
public class DictionaryResourceIT {
    ...
    void createDictionary() throws Exception {
        ...
        assertThat(testDictionary.getDelFlag()).isEqualTo(DEFAULT_DEL_FLAG);
        assertThat(testDictionary.getDeletedTime()).isEqualTo(DEFAULT_DELETED_TIME);
    }
    ...
    @Test
    @Transactional
    void getAllDictionariesByDelFlagIsEqualToSomething() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where delFlag equals to DEFAULT_DEL_FLAG
        defaultDictionaryShouldBeFound("delFlag.equals=" + DEFAULT_DEL_FLAG);

        // Get all the dictionaryList where delFlag equals to UPDATED_DEL_FLAG
        defaultDictionaryShouldNotBeFound("delFlag.equals=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllDictionariesByDelFlagIsInShouldWork() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where delFlag in DEFAULT_DEL_FLAG or UPDATED_DEL_FLAG
        defaultDictionaryShouldBeFound("delFlag.in=" + DEFAULT_DEL_FLAG + "," + UPDATED_DEL_FLAG);

        // Get all the dictionaryList where delFlag equals to UPDATED_DEL_FLAG
        defaultDictionaryShouldNotBeFound("delFlag.in=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllDictionariesByDelFlagIsNullOrNotNull() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where delFlag is not null
        defaultDictionaryShouldBeFound("delFlag.specified=true");

        // Get all the dictionaryList where delFlag is null
        defaultDictionaryShouldNotBeFound("delFlag.specified=false");
    }

    @Test
    @Transactional
    void getAllDictionariesByDeletedTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where deletedTime equals to DEFAULT_DELETED_TIME
        defaultDictionaryShouldBeFound("deletedTime.equals=" + DEFAULT_DELETED_TIME);

        // Get all the dictionaryList where deletedTime equals to UPDATED_DELETED_TIME
        defaultDictionaryShouldNotBeFound("deletedTime.equals=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllDictionariesByDeletedTimeIsInShouldWork() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where deletedTime in DEFAULT_DELETED_TIME or UPDATED_DELETED_TIME
        defaultDictionaryShouldBeFound("deletedTime.in=" + DEFAULT_DELETED_TIME + "," + UPDATED_DELETED_TIME);

        // Get all the dictionaryList where deletedTime equals to UPDATED_DELETED_TIME
        defaultDictionaryShouldNotBeFound("deletedTime.in=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllDictionariesByDeletedTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where deletedTime is not null
        defaultDictionaryShouldBeFound("deletedTime.specified=true");

        // Get all the dictionaryList where deletedTime is null
        defaultDictionaryShouldNotBeFound("deletedTime.specified=false");
    }

    @Test
    @Transactional
    void getAllDictionariesByDeletedTimeIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where deletedTime is greater than or equal to DEFAULT_DELETED_TIME
        defaultDictionaryShouldBeFound("deletedTime.greaterThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the dictionaryList where deletedTime is greater than or equal to UPDATED_DELETED_TIME
        defaultDictionaryShouldNotBeFound("deletedTime.greaterThanOrEqual=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllDictionariesByDeletedTimeIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where deletedTime is less than or equal to DEFAULT_DELETED_TIME
        defaultDictionaryShouldBeFound("deletedTime.lessThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the dictionaryList where deletedTime is less than or equal to SMALLER_DELETED_TIME
        defaultDictionaryShouldNotBeFound("deletedTime.lessThanOrEqual=" + SMALLER_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllDictionariesByDeletedTimeIsLessThanSomething() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where deletedTime is less than DEFAULT_DELETED_TIME
        defaultDictionaryShouldNotBeFound("deletedTime.lessThan=" + DEFAULT_DELETED_TIME);

        // Get all the dictionaryList where deletedTime is less than UPDATED_DELETED_TIME
        defaultDictionaryShouldBeFound("deletedTime.lessThan=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllDictionariesByDeletedTimeIsGreaterThanSomething() throws Exception {
        // Initialize the database
        dictionaryRepository.save(dictionary);

        // Get all the dictionaryList where deletedTime is greater than DEFAULT_DELETED_TIME
        defaultDictionaryShouldNotBeFound("deletedTime.greaterThan=" + DEFAULT_DELETED_TIME);

        // Get all the dictionaryList where deletedTime is greater than SMALLER_DELETED_TIME
        defaultDictionaryShouldBeFound("deletedTime.greaterThan=" + SMALLER_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllDictionariesByItemsIsEqualToSomething() throws Exception {
        CommonFieldData items;
        if (dictionaryRepository.findAll().isEmpty()) {
            dictionaryRepository.insert(dictionary);
            items = CommonFieldDataResourceIT.createEntity();
        } else {
            // items = commonFieldDataRepository.findAll().get(0);
            items = new CommonFieldData(); // todo
        }
    ...
    void putExistingDictionary() throws Exception {
        ...
        assertThat(testDictionary.getDelFlag()).isEqualTo(UPDATED_DEL_FLAG);
        assertThat(testDictionary.getDeletedTime()).isEqualTo(UPDATED_DELETED_TIME);
    }
    ...
    void partialUpdateDictionaryWithPatch() throws Exception {
        ...
        partialUpdatedDictionary.sortValue(UPDATED_SORT_VALUE).builtIn(UPDATED_BUILT_IN);
