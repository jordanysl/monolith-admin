...
import com.begcode.monolith.settings.web.rest.FillRuleItemResourceIT;
...
public class SysFillRuleResourceIT {
    ...
    void createSysFillRule() throws Exception {
        ...
        assertThat(testSysFillRule.getDelFlag()).isEqualTo(DEFAULT_DEL_FLAG);
        assertThat(testSysFillRule.getDeletedTime()).isEqualTo(DEFAULT_DELETED_TIME);
    }
    ...
    @Test
    @Transactional
    void getAllSysFillRulesByDelFlagIsEqualToSomething() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where delFlag equals to DEFAULT_DEL_FLAG
        defaultSysFillRuleShouldBeFound("delFlag.equals=" + DEFAULT_DEL_FLAG);

        // Get all the sysFillRuleList where delFlag equals to UPDATED_DEL_FLAG
        defaultSysFillRuleShouldNotBeFound("delFlag.equals=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllSysFillRulesByDelFlagIsInShouldWork() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where delFlag in DEFAULT_DEL_FLAG or UPDATED_DEL_FLAG
        defaultSysFillRuleShouldBeFound("delFlag.in=" + DEFAULT_DEL_FLAG + "," + UPDATED_DEL_FLAG);

        // Get all the sysFillRuleList where delFlag equals to UPDATED_DEL_FLAG
        defaultSysFillRuleShouldNotBeFound("delFlag.in=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllSysFillRulesByDelFlagIsNullOrNotNull() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where delFlag is not null
        defaultSysFillRuleShouldBeFound("delFlag.specified=true");

        // Get all the sysFillRuleList where delFlag is null
        defaultSysFillRuleShouldNotBeFound("delFlag.specified=false");
    }

    @Test
    @Transactional
    void getAllSysFillRulesByDeletedTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where deletedTime equals to DEFAULT_DELETED_TIME
        defaultSysFillRuleShouldBeFound("deletedTime.equals=" + DEFAULT_DELETED_TIME);

        // Get all the sysFillRuleList where deletedTime equals to UPDATED_DELETED_TIME
        defaultSysFillRuleShouldNotBeFound("deletedTime.equals=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSysFillRulesByDeletedTimeIsInShouldWork() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where deletedTime in DEFAULT_DELETED_TIME or UPDATED_DELETED_TIME
        defaultSysFillRuleShouldBeFound("deletedTime.in=" + DEFAULT_DELETED_TIME + "," + UPDATED_DELETED_TIME);

        // Get all the sysFillRuleList where deletedTime equals to UPDATED_DELETED_TIME
        defaultSysFillRuleShouldNotBeFound("deletedTime.in=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSysFillRulesByDeletedTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where deletedTime is not null
        defaultSysFillRuleShouldBeFound("deletedTime.specified=true");

        // Get all the sysFillRuleList where deletedTime is null
        defaultSysFillRuleShouldNotBeFound("deletedTime.specified=false");
    }

    @Test
    @Transactional
    void getAllSysFillRulesByDeletedTimeIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where deletedTime is greater than or equal to DEFAULT_DELETED_TIME
        defaultSysFillRuleShouldBeFound("deletedTime.greaterThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the sysFillRuleList where deletedTime is greater than or equal to UPDATED_DELETED_TIME
        defaultSysFillRuleShouldNotBeFound("deletedTime.greaterThanOrEqual=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSysFillRulesByDeletedTimeIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where deletedTime is less than or equal to DEFAULT_DELETED_TIME
        defaultSysFillRuleShouldBeFound("deletedTime.lessThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the sysFillRuleList where deletedTime is less than or equal to SMALLER_DELETED_TIME
        defaultSysFillRuleShouldNotBeFound("deletedTime.lessThanOrEqual=" + SMALLER_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSysFillRulesByDeletedTimeIsLessThanSomething() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where deletedTime is less than DEFAULT_DELETED_TIME
        defaultSysFillRuleShouldNotBeFound("deletedTime.lessThan=" + DEFAULT_DELETED_TIME);

        // Get all the sysFillRuleList where deletedTime is less than UPDATED_DELETED_TIME
        defaultSysFillRuleShouldBeFound("deletedTime.lessThan=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSysFillRulesByDeletedTimeIsGreaterThanSomething() throws Exception {
        // Initialize the database
        sysFillRuleRepository.save(sysFillRule);

        // Get all the sysFillRuleList where deletedTime is greater than DEFAULT_DELETED_TIME
        defaultSysFillRuleShouldNotBeFound("deletedTime.greaterThan=" + DEFAULT_DELETED_TIME);

        // Get all the sysFillRuleList where deletedTime is greater than SMALLER_DELETED_TIME
        defaultSysFillRuleShouldBeFound("deletedTime.greaterThan=" + SMALLER_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSysFillRulesByRuleItemsIsEqualToSomething() throws Exception {
        FillRuleItem ruleItems;
        if (sysFillRuleRepository.findAll().isEmpty()) {
            sysFillRuleRepository.insert(sysFillRule);
            ruleItems = FillRuleItemResourceIT.createEntity();
        } else {
            // ruleItems = fillRuleItemRepository.findAll().get(0);
            ruleItems = new FillRuleItem(); // todo
        }
    ...
    void putExistingSysFillRule() throws Exception {
        ...
        assertThat(testSysFillRule.getDelFlag()).isEqualTo(UPDATED_DEL_FLAG);
        assertThat(testSysFillRule.getDeletedTime()).isEqualTo(UPDATED_DELETED_TIME);
    }
