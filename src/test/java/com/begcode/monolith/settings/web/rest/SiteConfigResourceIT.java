...
public class SiteConfigResourceIT {
    ...
    void createSiteConfig() throws Exception {
        ...
        assertThat(testSiteConfig.getDelFlag()).isEqualTo(DEFAULT_DEL_FLAG);
        assertThat(testSiteConfig.getDeletedTime()).isEqualTo(DEFAULT_DELETED_TIME);
    }
    ...
    @Test
    @Transactional
    void getAllSiteConfigsByDelFlagIsEqualToSomething() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where delFlag equals to DEFAULT_DEL_FLAG
        defaultSiteConfigShouldBeFound("delFlag.equals=" + DEFAULT_DEL_FLAG);

        // Get all the siteConfigList where delFlag equals to UPDATED_DEL_FLAG
        defaultSiteConfigShouldNotBeFound("delFlag.equals=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllSiteConfigsByDelFlagIsInShouldWork() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where delFlag in DEFAULT_DEL_FLAG or UPDATED_DEL_FLAG
        defaultSiteConfigShouldBeFound("delFlag.in=" + DEFAULT_DEL_FLAG + "," + UPDATED_DEL_FLAG);

        // Get all the siteConfigList where delFlag equals to UPDATED_DEL_FLAG
        defaultSiteConfigShouldNotBeFound("delFlag.in=" + UPDATED_DEL_FLAG);
    }

    @Test
    @Transactional
    void getAllSiteConfigsByDelFlagIsNullOrNotNull() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where delFlag is not null
        defaultSiteConfigShouldBeFound("delFlag.specified=true");

        // Get all the siteConfigList where delFlag is null
        defaultSiteConfigShouldNotBeFound("delFlag.specified=false");
    }

    @Test
    @Transactional
    void getAllSiteConfigsByDeletedTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where deletedTime equals to DEFAULT_DELETED_TIME
        defaultSiteConfigShouldBeFound("deletedTime.equals=" + DEFAULT_DELETED_TIME);

        // Get all the siteConfigList where deletedTime equals to UPDATED_DELETED_TIME
        defaultSiteConfigShouldNotBeFound("deletedTime.equals=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSiteConfigsByDeletedTimeIsInShouldWork() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where deletedTime in DEFAULT_DELETED_TIME or UPDATED_DELETED_TIME
        defaultSiteConfigShouldBeFound("deletedTime.in=" + DEFAULT_DELETED_TIME + "," + UPDATED_DELETED_TIME);

        // Get all the siteConfigList where deletedTime equals to UPDATED_DELETED_TIME
        defaultSiteConfigShouldNotBeFound("deletedTime.in=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSiteConfigsByDeletedTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where deletedTime is not null
        defaultSiteConfigShouldBeFound("deletedTime.specified=true");

        // Get all the siteConfigList where deletedTime is null
        defaultSiteConfigShouldNotBeFound("deletedTime.specified=false");
    }

    @Test
    @Transactional
    void getAllSiteConfigsByDeletedTimeIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where deletedTime is greater than or equal to DEFAULT_DELETED_TIME
        defaultSiteConfigShouldBeFound("deletedTime.greaterThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the siteConfigList where deletedTime is greater than or equal to UPDATED_DELETED_TIME
        defaultSiteConfigShouldNotBeFound("deletedTime.greaterThanOrEqual=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSiteConfigsByDeletedTimeIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where deletedTime is less than or equal to DEFAULT_DELETED_TIME
        defaultSiteConfigShouldBeFound("deletedTime.lessThanOrEqual=" + DEFAULT_DELETED_TIME);

        // Get all the siteConfigList where deletedTime is less than or equal to SMALLER_DELETED_TIME
        defaultSiteConfigShouldNotBeFound("deletedTime.lessThanOrEqual=" + SMALLER_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSiteConfigsByDeletedTimeIsLessThanSomething() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where deletedTime is less than DEFAULT_DELETED_TIME
        defaultSiteConfigShouldNotBeFound("deletedTime.lessThan=" + DEFAULT_DELETED_TIME);

        // Get all the siteConfigList where deletedTime is less than UPDATED_DELETED_TIME
        defaultSiteConfigShouldBeFound("deletedTime.lessThan=" + UPDATED_DELETED_TIME);
    }

    @Test
    @Transactional
    void getAllSiteConfigsByDeletedTimeIsGreaterThanSomething() throws Exception {
        // Initialize the database
        siteConfigRepository.save(siteConfig);

        // Get all the siteConfigList where deletedTime is greater than DEFAULT_DELETED_TIME
        defaultSiteConfigShouldNotBeFound("deletedTime.greaterThan=" + DEFAULT_DELETED_TIME);

        // Get all the siteConfigList where deletedTime is greater than SMALLER_DELETED_TIME
        defaultSiteConfigShouldBeFound("deletedTime.greaterThan=" + SMALLER_DELETED_TIME);
    }
    ...
    void getAllSiteConfigsByItemsIsEqualToSomething() throws Exception {
        ...
        if (siteConfigRepository.findAll().isEmpty()) {
    ...
    void putExistingSiteConfig() throws Exception {
        ...
        assertThat(testSiteConfig.getDelFlag()).isEqualTo(UPDATED_DEL_FLAG);
        assertThat(testSiteConfig.getDeletedTime()).isEqualTo(UPDATED_DELETED_TIME);
    }
