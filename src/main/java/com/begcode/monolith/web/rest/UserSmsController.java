...
public class UserSmsController extends AuthenticateController {
        ...
        if (result) {
            ...
            if (userDTO.isPresent()) {
                ...
                if (userOptional.isPresent()) {
                    ...
                    String jwt = this.createTokenWithUserId(authenticationToken, rememberMe, userOptional.get().getId());
    ...
    public ResponseEntity<Result<String>> getSmsCodeByCurrentUser(String mobile, @RequestParam(name = "imageCode", required = false) String imageCode) {
        ...
        if (SecurityUtils.getCurrentUserLogin().isPresent()) {
            ...
            if (!existsMobile) {
                // 不存在，发送验证码
                Result<String> result = daYuSmsService.sendVerifyCode(mobile, "monolith-admin");
